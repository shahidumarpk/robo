              ...
              <table id="example1" class="display responsive nowrap" style="width:100%">
                ...
                @foreach($users as $user)
                  <tr>
                    ...
                    <form id="form{{$user['id']}}" action="{{action('InventoryController@destroy', $user['id'])}}" method="post">
                        @csrf
                        <input name="_method" type="hidden" value="DELETE">
                    </form>
                    <!-- For Delete Form Ends -->
                    <td>
                      <button href="" class="btn btn-primary" title="View Detail"><i class="fa fa-eye"></i> </button>    
                      ...
                      <!--
                      @if ($user['status'] === 1)
                        <a href="{!! url('/inventorys/deactivate/'.$user['id']); !!}"  class="btn btn-warning" title="Deactivate"><i class="fa fa-times"></i> </a>
                      @else
                        <a href="{!! url('/inventorys/active/'.$user['id']); !!}"  class="btn btn-info" title="Active"><i class="fa fa-check"></i> </a>
                      @endif
                       <button class="btn btn-danger" onclick="archiveFunction('form{{$user['id']}}')"><i class="fa fa-trash"></i></button>
                      <a href="{!! url('/inventorys/resetpassword/'.$user->id); !!}"  class="btn btn-info" title="Reset Password"><i class="fa fa-key"></i> </a> -->
                    </td>
